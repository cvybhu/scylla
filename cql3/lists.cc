...
lists::do_append(shared_ptr<term> value,
    ...
    if (column.type->is_multi_cell()) {
        ...
        if (!std::holds_alternative<expr::constant_value>(expr_val)) {
            return;
        }

        auto ltype = static_cast<const list_type_impl*>(column.type.get());

        auto&& to_add = expr::get_list_elements(std::get<expr::constant_value>(expr_val));
        ...
        if (!value) {
            ...
            auto newv = managed_bytes_view(std::get<expr::constant_value>(expr_val).value_bytes);
...
lists::prepender::execute(mutation& m, const clustering_key_prefix& prefix, const update_parameters& params) {
    ...
    const expr::constant_value& lvalue = std::get<expr::constant_value>(value);
    assert(lvalue.value_type->is_list());
...
lists::discarder_by_index::execute(mutation& m, const clustering_key_prefix& prefix, const update_parameters& params) {
    ...
    expr::expression_value index = expr::evaluate(_t, params._options);
    if (std::holds_alternative<expr::null>(index)) {
        throw exceptions::invalid_request_exception("Invalid null value for list index");
    }
    if (std::holds_alternative<expr::unset>(index)) {
        return;
    }

    auto&& existing_list_opt = params.get_prefetched_list(m.key(), prefix, column);
    int32_t idx = expr::get_raw_view(index).deserialize<int32_t>(*int32_type);
