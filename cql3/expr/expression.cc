...
extern std::vector<expression> extract_column_restrictions(const expression& expr, const column_definition& column) {
    struct visitor {
        ...
        void operator()(const binary_operator& oper) {
            if (current_binary_operator != nullptr) {
                on_internal_error(expr_logger, "extract_column_restrictions: nested binary operators are not supported");
            }
        ...
        void operator()(const column_value_tuple& tuple) {
            for (const column_value& cv : tuple.elements) {
                if (*cv.col == column && current_binary_operator != nullptr) {
                    restrictions.emplace_back(*current_binary_operator);
                    break;
                }
