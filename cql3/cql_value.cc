...
namespace cql3 {
    ...
    std::vector<managed_bytes_opt> tuple_value::get_serialized_elements() const {
        std::vector<managed_bytes_opt> result;
        for (const cql_value& elem : elements) {
            result.emplace_back(elem.to_raw_value().to_bytes());
        }
    }

    std::vector<managed_bytes_opt> list_value::get_serialized_elements() const {
        std::vector<managed_bytes_opt> result;
        for (const cql_value& elem : elements) {
            result.emplace_back(elem.to_raw_value().to_bytes());
        }
