...
class filtering_query_pager : public query_pager {
    const ::shared_ptr<const cql3::restrictions::refactor_restrictions> _filtering_restrictions;
public:
    filtering_query_pager(service::storage_proxy& p, schema_ptr s, shared_ptr<const cql3::selection::selection> selection,
                ...
                lw_shared_ptr<query::read_command> cmd,
                dht::partition_range_vector ranges,
                ::shared_ptr<const cql3::restrictions::refactor_restrictions> filtering_restrictions)
...
std::unique_ptr<service::pager::query_pager> service::pager::query_pagers::pager(
    ...
    if (!filtering_restrictions && cmd->slice.partition_row_limit() < query::max_rows_if_set) {
        filtering_restrictions = ::make_shared<const cql3::restrictions::refactor_restrictions>(::make_shared<cql3::restrictions::statement_restrictions>(s, true));
    }
